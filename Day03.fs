...
let input = File.ReadAllLines(@"C:/Source/AoC2019/inputs/3test.txt")
...
let Part1 = 
    ...
    let findShortestIntersection(inputGrid) =
        let absSumTuple(tup) = 
            let t1, t2 = tup
            abs(t1) + abs(t2)
        ...
        let shortestIntersection = inputGrid |> Map.filter (fun _ v -> v > 1)  // Find intersections
